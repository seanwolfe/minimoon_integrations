...
class MmAnalyzer:
        ...
        for root, dirs, files in os.walk(population_dir):
            ...
            if name in files:
                ...
                results = [stc, time_SOI_EMS, peri_in_SOI_EMS, peri_in_3_hill, peri_in_2_hill, peri_in_1_hill,
                           ...
                           ems_end_idx, ems_start, ems_start_idx, ems_end, ems_end_idx, TCO_state, Earth_state, moon_state]
    ...
    def alpha_beta_jacobi(self, object_id):
        ...
        # translation
        C_T_h = np.array([0.01, 0., 0.])

        # in sun-earth/moon corotating
        C_r_TCO = C_R_h @ h_r_TCO + C_T_h

        # v_rel in Jacobi constant
        v_rel = np.linalg.norm(h_v_TCO - v_C + np.cross(np.array([omega, 0, 0]), ))
        ...
        alpha = np.atan((y - ems_barycentre[1]) / (x - ems_barycentre[0]))

        results = [C_J_dimensional, C_J_nondimensional, alpha, beta]
