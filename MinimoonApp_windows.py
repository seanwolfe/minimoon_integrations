...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        data = self.create_parser_windows()
        new_data = data
        ...
        self.w_title = QLabel(str(self.mm_parser.mm_data["Object id"].iloc[0]))  # make title of bottom right the minimoon name
        ...
        list_widget.addItems(self.mm_parser.mm_data["Object id"])  # all the minimoon names
        list_widget.currentItemChanged.connect(self.index_changed_windows)  # this is the callback for when the user clicks a
    ...
    def create_parser(self):
        ...
        mm_file_name = self.mm_parser.mm_data["Object id"].iloc[2] + ".dat"
        mm_file_name_new = self.mm_parser.mm_data["Object id"].iloc[2] + ".csv"
        temp_file = destination_path + '/' + mm_file_name
        temp_file_new = destination_path_new + '/' + mm_file_name_new
        header = None
        data = self.mm_parser.mm_file_parse(temp_file, header)
    ...
    def create_parser_windows(self):
        ...
        mm_master_file_name = 'minimoon_master.csv'  # name of the minimoon file to parsed
        ...
        # organize the data in the minimoon_file
        self.mm_parser.organize_data_new(header)
        ...
        data = self.mm_parser.mm_file_parse_new(temp_file_new, header)
