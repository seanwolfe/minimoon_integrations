...
def minimoon_check(ephs, x_obs, y_obs, z_obs, vx_obs, vy_obs, vz_obs, grav_param, start_time):
    """
    ...
    time_satisfied_1 = np.zeros((N, 1))  # The number of days for which the condition 1 was satisfied

    # Variables to provide information of condition 2 (see function definition)
    satisfied_2 = np.zeros((N, days))  # ==1 when condition two is satisfied ==0 otherwise
    time_satisfied_2 = np.zeros((N, 1))   # The number of days for which condition two is satisfied
    epsilons = np.zeros((N, days))  # Specific energy relative to Earth
    vs_rel = np.zeros((N, days))  # Relative velocity of asteroid wrt to Earth
    # This is the same as 'distances': rs_rel_e = np.zeros((N, days))  # Relative distance of asteroid wrt to Earth
    ...
    captured = np.zeros((N, days))  # When the asteroid is captured (i.e. condition 1 and 2 met)
    ...
    for i in range(0, N):
        ...
        epsilon_j = np.zeros((1, days))
        v_rel_j = np.zeros((1, days))

        for j in range(0, days):
            # Body-centered x,y,z of minimoon from openorb data
            x = ephs[i][j, 24] - x_obs[j]
            y = ephs[i][j, 25] - y_obs[j]
            z = ephs[i][j, 26] - z_obs[j]
            ...
            if d < three_eh:
                satisfied_1[i, j] = 1
                time_satisfied_1[i, 0] += 1

                if d < one_eh:
                    satisfied_4[i, j] = 1
                    satisfied_4_overall[i, 0] = 1
                    time_satisfied_4[i, 0] += 1

            # For velocity in observer-centered frame - in meters per second
            vx = ((ephs[i][j, 27] - vx_obs[j]) * aupd).to(mps) / mps
            vy = ((ephs[i][j, 28] - vy_obs[j]) * aupd).to(mps) / mps
            vz = ((ephs[i][j, 29] - vz_obs[j]) * aupd).to(mps) / mps
            ...
            if epsilon < 0:
                satisfied_2[i, j] = 1
                time_satisfied_2[i, 0] += 1

            # Identify beginning of capture
            if satisfied_1[i, j] == 1 and satisfied_2[i, j] == 1:
                prev_capture = 1
                # Store date of capture
                if first_capture == 0:
                    capture_date = strt_tm + j * u.day
                    first_capture = 1
                captured[i, j] = 1
                time_captured[i, 0] += 1

            # Identify end of capture
            if satisfied_1[i, j] == 0 and satisfied_2[i, j] == 0 and prev_capture == 1:
                # Store date of end of capture
                prev_capture = 0
                release_date = strt_tm + j * u.day

            # Check to see how many revolutions were made during capture phase
            if captured[i, j] == 1 and j > 0:
                if captured[i, j-1] == 1:
                    curr_angle = ephs[i][j, 11]  # Grab ecliptic longitude in the topocentric frame (== earth-centered
                    # if observer is at center of Earth
                    prev_angle = ephs[i][j-1, 11]
                    diff_angle = curr_angle - prev_angle  # how much object moved
                    if abs(diff_angle) > 180:
                        diff_angle = -360 + diff_angle
                    cum_angle[i, 0] += diff_angle
                else:
                    cum_angle[i, 0] = 0  # reset if previously uncaptured

        distances[i, :] = distance[0, :]
        vs_rel[i, :] = v_rel_j[0, :]
        epsilons[i, :] = epsilon_j[0, :]
        min_distances[i, 0] = min(distance[0, :])
        revolutions[i, 0] = abs(cum_angle / 360.0)
        if revolutions[i, 0] >= 1:
            satisfied_3[i, 0] = 1

    for i in range(0, N):
        if time_captured > 0 and satisfied_3[i, 0] == 1 and satisfied_4_overall[i, 0] == 1:
            ...
            print("Duration (days) of Temporary capture: " + str(time_captured[i, 0]))
        else:
            print("Object became minimoon: NO")
        print("Time spent (days) closer than 3 Earth Hill Radii: " + str(time_satisfied_1[i, 0]))
        print("Time spent (days) with specific energy less than zero (wrt to observer): " + str(time_satisfied_2[i, 0]))
        if satisfied_3[i, 0] == 1:
            print("Completed at least a revolution: YES")
        else:
            print("Completed at least a revolution: NO")
        print("Number of revolutions completed: " + str(revolutions[i, 0]))
        if satisfied_4_overall[i, 0] == 1:
            print("Approached to within 1 Earth Hill radius: YES")
            print("Time spent (days) close than 1 Earth Hill radius: " + str(time_satisfied_4[i, 0]))
        else:
            print("Approached to within 1 Earth Hill radius: NO")

        print("Minimum distance reached to observer (AU): " + str(min_distances[i, 0]))
        print("\n")

    return N

def minimoon_check_jpl(x, y, z, vx, vy, vz, grav_param, start_time, ecl_lon):
    """
    ...
    # Grab the geocentric cartesian coordinates of the test particle
    days = len(x)  # Number of days in the integration
    print(days)
